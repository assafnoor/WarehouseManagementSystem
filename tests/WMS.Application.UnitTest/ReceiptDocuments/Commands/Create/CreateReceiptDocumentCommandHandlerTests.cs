...
//        _receiptDocumentRepoMock.Setup(r => r.ExistsByNumberAsync(DocumentNumber.CreateNew(command.DocumentNumber))).ReturnsAsync(true);
...
//        _receiptDocumentRepoMock.Setup(r => r.ExistsByNumberAsync(DocumentNumber.CreateNew(command.DocumentNumber))).ReturnsAsync(false);

//        // Resource exists but UoM does not exist
//        _resourceRepoMock.Setup(r => r.ExistsActiveAsync(It.IsAny<Recou>())).ReturnsAsync(true);
...
//        _receiptDocumentRepoMock.Setup(r => r.AddAsync(It.IsAny<WMS.Domain.ReceiptDocumentAggregate.ReceiptDocument>())).Returns(Task.CompletedTask);
